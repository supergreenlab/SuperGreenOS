...
static uint8_t buf_out[MAX_QUEUE_ITEM_SIZE] = {0};
  ...
  static char scr_channel[MAX_KVALUE_SIZE] = {0};
...
static void mqtt_task(void *param) {
  ...
  ESP_LOGI(SGO_LOG_EVENT, "@MQTT Log channel: %s", log_channel);
    ...
    const char scr_token[MAX_KVALUE_SIZE];
    get_broker_scrtoken(scr_token, sizeof(scr_token)-1);
    snprintf(scr_channel, sizeof(scr_channel)-1, "%s.scrcmd/%s", client_id, scr_token);
    ESP_LOGI(SGO_LOG_EVENT, "@MQTT Scr channel: %s", log_channel);
      ...
      while (xQueueReceive(log_queue, buf_out, 0)) {
        ...
        memset(buf_out, 0, MAX_QUEUE_ITEM_SIZE);
				...
				while (xQueueReceive(scr_queue, buf_out, 0)) {
					int len = strnlen((char *)buf_out, MAX_QUEUE_ITEM_SIZE);
					len = (int)(ceilf((float)len/16.0f)*16);
					ESP_LOGI(SGO_LOG_NOSEND, "Strlen: %d", len);
          memset(msgenc, 0, sizeof(msgenc));
          aes_encrypt_msg((const unsigned char *)buf_out, len, msgenc, key);
