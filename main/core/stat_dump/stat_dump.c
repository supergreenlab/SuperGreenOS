...
static void stat_dump_task(void *param) {
  ...
  while(1) {
    ...
    if ((counter % 20) == 0 || is_status_led_red_gpio_changed()) {
      value = get_status_led_red_gpio();
      ESP_LOGI(SGO_LOG_METRIC, "@KV %s=%d", "STATUS_LED_RED_GPIO", value);
      vTaskDelay(200 / portTICK_PERIOD_MS);
      reset_status_led_red_gpio_changed();
    }
    if ((counter % 20) == 0 || is_status_led_green_gpio_changed()) {
      value = get_status_led_green_gpio();
      ESP_LOGI(SGO_LOG_METRIC, "@KV %s=%d", "STATUS_LED_GREEN_GPIO", value);
      vTaskDelay(200 / portTICK_PERIOD_MS);
      reset_status_led_green_gpio_changed();
    }
    if ((counter % 20) == 0 || is_status_led_dim_changed()) {
      value = get_status_led_dim();
      ESP_LOGI(SGO_LOG_METRIC, "@KV %s=%d", "STATUS_LED_DIM", value);
      vTaskDelay(200 / portTICK_PERIOD_MS);
