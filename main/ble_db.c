...
#define LED_CHAR(i) RW_I_NOTIFIABLE_CHAR(LED_##i##_DUTY)

#define LED_CB(i)  else if (param->write.handle == handle_table[IDX_VALUE(LED_##i##_DUTY)]) { \
    on_set_led_duty(IDX_VALUE(LED_##i##_DUTY), i, *(uint32_t *)(&param->write.value[0])); \
  }
