...
int get_output_for_time(int boxId) {
  ...
  double output = -(cos(year_adv * M_PI * 2) * 0.25 - cos((day_adv-0.5) * M_PI * 2) * ((-cos(year_adv * M_PI * 2) + 1) / 2 + 1) - 1);
  double output = -cos(year_adv*M_PI*2)*0.25 + (-cos(day_adv*M_PI*2)) + 0.3 + -cos(year_adv*M_PI*2)*0.2;
