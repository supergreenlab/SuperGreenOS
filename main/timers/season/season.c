...
int get_output_for_time() {
  ...
  ESP_LOGI(SGO_LOG_EVENT, "@SEASON timeinfo.tm_hour = %d, timeinfo.tm_min = %d, timeinfo.tm_sec = %d, year_adv = %f, day_adv = %f, output = %f", timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec, year_adv, day_adv, output);
  return min(100, max(0, output * 100));
}

void season_task() {
  time_t box_time = get_box_time();
  ESP_LOGI(SGO_LOG_EVENT, "@SEASON Simulated time = %lu", box_time);
